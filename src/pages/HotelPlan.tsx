// pages/hotel/HotelCalendarPage.tsx
...
// ── Types (miroir exact du schéma Prisma) ────────────────────────────────────
...
const RES_STYLE: Record<ReservationStatus, { tw: string; dot: string; label: string }> = {
  checked_in:  { tw:"bg-blue-500 text-white",    dot:"bg-blue-500",    label:"Arrivé"   },
  booked:      { tw:"bg-violet-500 text-white",  dot:"bg-violet-500",  label:"Réservé"  },
  checked_out: { tw:"bg-emerald-500 text-white", dot:"bg-emerald-500", label:"Parti"    },
  cancelled:   { tw:"bg-slate-400 text-white",   dot:"bg-slate-400",   label:"Annulé"   },
  no_show:     { tw:"bg-red-400 text-white",     dot:"bg-red-400",     label:"No-show"  },
};

const MAINT_STYLE: Record<MaintenanceStatus, { tw: string; dot: string; label: string }> = {
  scheduled:   { tw:"bg-orange-400 text-white",  dot:"bg-orange-400",  label:"Planifiée" },
  in_progress: { tw:"bg-red-500 text-white",     dot:"bg-red-500",     label:"En cours"  },
  completed:   { tw:"bg-emerald-400 text-white", dot:"bg-emerald-400", label:"Terminée"  },
  cancelled:   { tw:"bg-slate-400 text-white",   dot:"bg-slate-400",   label:"Annulée"   },
};

const ROOM_STATUS: Record<RoomStatus, { dot: string; label: string }> = {
  available:    { dot:"bg-emerald-500", label:"Disponible"   },
  occupied:     { dot:"bg-blue-500",    label:"Occupée"      },
  cleaning:     { dot:"bg-amber-400",   label:"Nettoyage"    },
  maintenance:  { dot:"bg-red-500",     label:"Maintenance"  },
  out_of_order: { dot:"bg-slate-500",   label:"Hors service" },
};
...
function BarPopover({ bar }: { bar: Bar }) {
  ...
  const label  = isRes
    ...
    : ((bar.data as RoomMaintenance).reason ?? "Maintenance");

  // ── Libellé du tarif selon rateMode (titre uniquement) ────────────────────
  const rateLabel = isRes
    ? (() => {
        const res = bar.data as Reservation;
        return res.rateMode === "per_stay"
          ? `${fmtMGA(res.rate)} MGA/séjour`
          : `${fmtMGA(res.rate)} MGA/nuit`;
      })()
    : null;
          ...
          <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
            {isRes ? (
              <>
                ...
                {/* rateMode = titre uniquement — per_stay → MGA/séjour, per_night → MGA/nuit */}
                <span className="font-medium">{rateLabel}</span>
                <span className="text-muted-foreground">Statut</span>
                <Badge variant="secondary" className="text-[10px] h-4 px-1.5 w-fit">{style.label}</Badge>
              </>
            ) : (
              <>
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto p-4 sm:p-6">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
              <div>
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground mb-1">Hôtel · Gestion</p>
                <h1 className="text-xl sm:text-2xl font-semibold tracking-tight">Calendrier des Chambres</h1>
              </div>
                  ...
                  <Card>
                    <div className="px-4 py-2.5 text-center min-w-[110px]">
                      <p className={`text-xl font-bold tabular-nums ${
                        todayAvail > totalRooms * 0.5 ? "text-emerald-500" : "text-amber-500"
                      }`}>
                        {todayAvail}
                        <span className="text-xs font-normal text-muted-foreground">/{totalRooms}</span>
                      </p>
                      <p className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-wide">Dispo. ce jour</p>
                    </div>
                  </Card>
                )}
                <Card>
                  <div className="px-4 py-2.5 text-center min-w-[110px]">
                    {isLoading
                      ...
                      : <p className="text-xl font-bold text-blue-500 tabular-nums">{activeRes}</p>
                    }
                    <p className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-wide">Réservations</p>
                  </div>
                </Card>
                <Card>
                  <div className="px-4 py-2.5 text-center min-w-[110px]">
                    {isLoading
                      ? <Skeleton className="h-7 w-10 mx-auto mb-1" />
                      : <p className="text-xl font-bold text-red-500 tabular-nums">{activeMaint}</p>
                    }
                    <p className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-wide">Maintenances</p>
                  </div>
                </Card>
              </div>
            </div>
            ...
            {isError && (
              <div className="flex items-center gap-3 p-4 mb-4 rounded-lg border border-destructive/30 bg-destructive/10 text-destructive text-sm">
                ...
                <span>Impossible de charger les données. Vérifiez votre connexion au serveur.</span>
            ...
            <div className="flex items-center gap-2 mb-4">
              <Button variant="outline" size="sm" onClick={prevMonth}>← Préc.</Button>
              <span className="text-sm font-semibold min-w-[150px] text-center tabular-nums">
                {MONTHS_FR[viewMonth]} {viewYear}
              </span>
              <Button variant="outline" size="sm" onClick={nextMonth}>Suiv. →</Button>
              <Button
                  ...
                  <div style={{ minWidth: "820px" }}>
                          ...
                          <Tooltip key={i}>
                            <TooltipTrigger asChild>
                              <div className={`flex flex-col items-center pt-1.5 pb-1 select-none ${isWeekend ? "bg-muted/30" : ""}`}>
                                ...
                                <span className={`text-[11px] font-medium w-5 h-5 flex items-center justify-center rounded-full mt-0.5 ${
                                  isToday
                                ...
                                <div className="w-full h-[3px] mt-1 bg-muted overflow-hidden">
                                  <div
                                    className={`h-full transition-all ${ratio>.6?"bg-emerald-500":ratio>.3?"bg-amber-400":"bg-red-500"}`}
                            ...
                            <TooltipContent side="bottom" className="text-xs">
                              {dailyAvail[i]}/{totalRooms} disponible{dailyAvail[i] > 1 ? "s" : ""}
                            </TooltipContent>
                          </Tooltip>
                        );
                      })}
                    ...
                    {rooms.map(room => {
                      ...
                      return (
                        <div
                          key={room.id}
                          className="grid border-b border-border/50 last:border-0 hover:bg-muted/20 transition-colors"
                          style={{ gridTemplateColumns: cols, height: "42px", position: "relative" }}
                        >
                          <div className="flex items-center gap-2 px-3 z-10">
                            ...
                            <Badge variant="outline" className="text-[10px] px-1 py-0 h-4 font-normal hidden sm:inline-flex">
                              {room.type}
                            </Badge>
                          </div>
                          ...
                          {daysInMonth.map((day, i) => {
                            ...
                            return (
                              <div key={i} className={`h-full border-l border-border/25
                                ${isToday ? "bg-primary/5 border-l-primary/30" : ""}
                          ...
                          <div
                            className="absolute inset-y-[5px] pointer-events-none"
                            style={{
                              left: "160px", right: 0,
                              display: "grid",
                          ...
                          >
                            {bars.map((bar, bi) => <BarPopover key={bi} bar={bar} />)}
                          </div>
                        </div>
                      );
                    })}
                        ...
                        return (
                          <Tooltip key={i}>
                            <TooltipTrigger asChild>
                              <div className={`flex flex-col items-center justify-center py-1.5 cursor-default ${isToday ? "bg-primary/5" : ""}`}>
                                <span className={`text-[11px] font-semibold tabular-nums ${
                                  ratio>.6?"text-emerald-500":ratio>.3?"text-amber-500":"text-red-500"
                                }`}>
                                  {avail}
                                </span>
                                <div className="w-full px-0.5 mt-0.5">
                                  <div className="h-[3px] rounded-full bg-muted overflow-hidden">
                                    <div
                                      className={`h-full rounded-full ${ratio>.6?"bg-emerald-500":ratio>.3?"bg-amber-400":"bg-red-500"}`}
                                      style={{ width: `${ratio * 100}%` }}
                                    />
                                  </div>
                                </div>
                            ...
                            <TooltipContent side="top" className="text-xs">
                              {avail}/{totalRooms} chambre{avail > 1 ? "s" : ""} disponible{avail > 1 ? "s" : ""}
                            </TooltipContent>
                          </Tooltip>
                        );
                      })}
            ...
            {!isLoading && !isError && (
              <div className="mt-5 flex flex-wrap gap-6">
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-2">Réservations</p>
                ...
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-2">Maintenance</p>
                  <div className="flex flex-wrap gap-3">
                ...
                <Separator orientation="vertical" className="hidden sm:block self-stretch h-auto" />
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase tracking-widest mb-2">Statut chambre</p>
                  <div className="flex flex-wrap gap-3">
